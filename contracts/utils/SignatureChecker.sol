...
abstract contract SignatureChecker is Ownable2Step {
    ...
    /**
     * @notice Initializes the contract with the specified signer address and creates the domain
     * separator.
     * @param signer The address of the signer.
     * @dev The signer address cannot be the zero address.
     * @dev The domain separator is generated using the EIP-712 standard and includes the contract's
    ...
    function changeSigner(address newSigner) external onlyOwner returns (bool) {
        require(newSigner != address(0), "New signer address cannot be a zero address");
        _signer = newSigner;
        return true;
    }
     ...
     */
    function recover(Presale memory presale, bytes memory signature) public view returns (bool) {
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01", _domainSeperator, keccak256(abi.encode(_PERMIT_TYPEHASH, presale))
            )
        );
     ...
     */
    function split(bytes memory signature) public pure returns (uint8, bytes32, bytes32) {
